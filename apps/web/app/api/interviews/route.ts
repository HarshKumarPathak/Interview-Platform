...
export async function POST(request: Request) {
  try {
    ...
    if (!allowedTypes.has(type) || !allowedDifficulty.has(difficulty) || !allowedLanguages.has(language) || ![15, 30, 45, 60].includes(durationMinutes) || ![1, 2, 3].includes(panelSize)) {
  ...
  try {
    ...
    const result = await query<{ id: string; status: string }>(
      `update interviews set status = $2::interview_status,
       ...
       where id = $1 and candidate_id = $3 returning id, status`,
      [interviewId, status, candidateId],
    );
