...
export async function POST(request: Request) {
  try {
    ...
    const candidate = await query<{ id: string; display_name: string }>(
      "select c.id, c.display_name from candidates c where c.user_id = $1",
      [session.userId],
    );
    const candidateRow = candidate.rows[0];
    if (!candidateRow) return NextResponse.json({ error: "Candidate not found" }, { status: 404 });

    const interview = await query<{ id: string; candidate_id: string; status: string }>(
      "select id, candidate_id, status from interviews where id = $1 and candidate_id = $2",
      [interviewId, candidateRow.id],
    );
    if (!interview.rows[0]) return NextResponse.json({ error: "Interview not found" }, { status: 404 });
    if (!["ready", "in_progress"].includes(interview.rows[0].status)) {
      return NextResponse.json({ error: "Interview is not joinable" }, { status: 409 });
    }
    ...
    const token = createLiveKitToken({
      apiKey,
      apiSecret,
      identity: `candidate-${candidateRow.id}`,
      name: candidateRow.display_name,
      room,
    });

    return NextResponse.json({ token, serverUrl, room });
  } catch (error) {
