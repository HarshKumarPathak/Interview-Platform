...
export default async function AdminPage() {
  ...
  return <main className="product-shell">
    ...
    <section className="dashboard container">
      ...
      <section className="panel recent-panel"><div className="panel-heading"><div><span className="eyebrow">Users</span><h2>Account activity</h2></div><span>{usersResult.rows.length} shown</span></div>{usersResult.rows.map((user) => <div className="history-row" key={user.user_id}><div><strong>User {user.user_id.slice(0, 8)}</strong><span>Joined {formatDate(user.joined_at)} · {user.interview_count} interviews</span></div><span className="status">{user.login_count} logins</span><b>{formatDate(user.last_login)}</b></div>)}</section>
      <section className="panel recent-panel"><div className="panel-heading"><div><span className="eyebrow">Authentication</span><h2>Recent successful logins</h2></div><span>Latest 20</span></div>{activityResult.rows.map((event) => <div className="history-row" key={`${event.user_id}-${event.created_at}`}><div><strong>User {event.user_id.slice(0, 8)}</strong><span>Successful authentication</span></div><span className="status status-done">Login</span><b>{formatDate(event.created_at)}</b></div>)}</section>
    </section>
  </main>;
}
