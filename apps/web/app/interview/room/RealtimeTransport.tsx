...
type LiveKitClient = {
  ...
  RoomEvent: { TrackSubscribed: string; TrackUnsubscribed: string; ActiveSpeakersChanged: string; ParticipantAttributesChanged: string; ParticipantConnected: string; ParticipantDisconnected: string; ConnectionQualityChanged: string };
...
function qualityLabel(quality: ConnectionQuality) {
  ...
  if (quality === "poor") return "Connection issue · fallback ready";
...
export default function RealtimeTransport({ interviewId, stream, muted, screenShareEnabled = false, onStatus, onScreenShareStatus, onRemoteVideoTrack, onRemoteVideoTrackRemoved, onActiveSpeaker, onInterviewerState, onConnectionQuality }: Props) {
  ...
  useEffect(() => {
    ...
    const currentInterviewId = interviewId; const activeStream = stream; let active = true; let room: LiveKitRoom | null = null; let client: LiveKitClient | null = null; const remoteAudioElements: HTMLMediaElement[] = []; const remoteVideoParticipants = new Set<string>();
    ...
    async function connect() {
      ...
      try {
        ...
        const payload = await tokenResponse.json() as { token?: unknown; serverUrl?: unknown }; const token = typeof payload.token === "string" ? payload.token : null; const serverUrl = typeof payload.serverUrl === "string" ? payload.serverUrl : null;
        ...
        const onParticipantAttributesChanged = (...args: unknown[]) => reportParticipantState(args[1] as LiveKitRemoteParticipant | undefined);
  ...
  return <span style={{ position: "fixed", top: 88, right: 16, zIndex: 30, padding: "7px 10px", borderRadius: 999, background: "rgba(8,21,35,.92)", border: "1px solid #29405a", color: "#a9b9ca", fontSize: 10, boxShadow: "0 8px 24px rgba(0,0,0,.24)" }} aria-live="polite">{status} · {qualityLabel(connectionQuality)}</span>;
}
