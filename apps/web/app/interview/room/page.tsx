...
export default function InterviewRoomPage() {
  ...
  useEffect(() => {
    ...
    async function startCamera() { try { const mediaStream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true }); if (!active) { mediaStream.getTracks().forEach((track) => track.stop()); return; } streamRef.current = mediaStream; setStream(mediaStream); if (videoRef.current) { videoRef.current.srcObject = mediaStream; await videoRef.current.play(); } setCameraReady(true); } catch { setCameraReady(false); } }
    void startCamera(); return () => { active = false; stopListening(); streamRef.current?.getTracks().forEach((track) => track.stop()); streamRef.current = null; setStream(null); if (typeof window !== "undefined" && "speechSynthesis" in window) window.speechSynthesis.cancel(); };
  }, [stopListening]);
  ...
  useEffect(() => {
    async function start() {
      try {
        ...
        const lifecycleResponse = await fetch("/api/interviews", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ interviewId: payload.interview.id, status: "in_progress" }) }); if (!lifecycleResponse.ok) throw new Error("interview start failed");
        setLoading(false);
      } catch (error) { console.error(error); const fallback = "Tell me about a project you are most proud of and the impact you had on it."; setQuestion(fallback); setQuestionMeta({ question: fallback, stage: "intro", question_index: 0, role: "technical_interviewer", source: "fallback" }); setLoading(false); }
    }
    void start();
  ...
  useEffect(() => {
    ...
    async function startFallback() {
      try {
        const interviewType = String(config.type ?? "placement").toLowerCase(); const difficulty = (config.difficulty ?? "adaptive").toLowerCase(); const language = config.language ?? "English";
        ...
        const ai: AIQuestion = await response.json(); setQuestion(ai.question); setQuestionMeta(ai); lastTurnSequenceRef.current = 0; speakQuestion(ai.question);
    ...
    void startFallback();
  }, [candidate, config, interviewId, realtimeStatus, speakQuestion]);

  useEffect(() => {
    if (!interviewId || realtimeStatus !== "connected") return;
    let active = true;
    const syncTurns = async () => {
      try {
        ...
        for (const turn of turns) {
          if (turn.sequence_no <= lastTurnSequenceRef.current) continue;
          lastTurnSequenceRef.current = turn.sequence_no;
          if (turn.speaker === "interviewer") {
            const metadata = turn.metadata ?? {};
            setQuestion(turn.content);
            setQuestionMeta({
              question: turn.content,
              role: turn.role ?? undefined,
              source: typeof metadata.question_source === "string" ? metadata.question_source : typeof metadata.source === "string" ? metadata.source : "livekit",
              ...
              question_index: typeof metadata.question_index === "number" ? metadata.question_index : undefined,
              policy_focus: Array.isArray(metadata.policy_focus) ? metadata.policy_focus.filter((value): value is string => typeof value === "string") : undefined,
            });
            setAnswerCount((count) => Math.max(count, turns.filter((item) => item.speaker === "candidate").length));
          }
        }
      } catch (error) { console.warn("Realtime transcript sync failed", error); }
    };
    void syncTurns();
    const interval = window.setInterval(() => void syncTurns(), 1200);
    return () => { active = false; window.clearInterval(interval); };
  }, [interviewId, realtimeStatus]);
