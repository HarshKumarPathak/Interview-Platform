...
type CandidateContext = {
  candidate: { id: string; display_name?: string; headline?: string; college?: string; degree?: string; graduation_year?: number };
...
export default function InterviewRoomPage() {
  ...
  useEffect(() => {
    ...
    void startCamera();
    return () => { active = false; stopListening(); streamRef.current?.getTracks().forEach((track) => track.stop()); streamRef.current = null; if (typeof window !== "undefined" && "speechSynthesis" in window) window.speechSynthesis.cancel(); };
  }, [stopListening]);
  ...
  useEffect(() => {
    async function start() {
      try {
        ...
        setQuestion(fallback); setQuestionMeta({ stage: "intro", question_index: 0, role: "technical_interviewer", source: "fallback" }); setLoading(false); speakQuestion(fallback);
      }
    }
    void start();
  ...
  return (
    <main className="room">
      ...
      <section className="room-stage">
        <div className="interviewer-stage"><div className="avatar-face large"><span className="eye left" /><span className="eye right" /><span className="mouth" /></div><div className="interviewer-name"><strong>{interviewerName}</strong><span>{config.type === "hr" ? "HR Interviewer · AI Panel" : `${questionMeta?.role?.replaceAll("_", " ") ?? "Senior Software Engineer"} · AI Interviewer`}</span></div></div>
        <div className="candidate-video">{cameraReady ? <video ref={videoRef} muted playsInline autoPlay /> : <div className="candidate-placeholder">{candidate?.candidate?.display_name?.[0]?.toUpperCase() ?? "Y"}</div>}<span>You · {cameraReady ? "camera live" : "camera unavailable · text fallback"}</span></div>
        <div className="room-question"><span>INTERVIEWER · {answerCount + 1} / {maxAnswers}{questionMeta?.stage ? ` · ${questionMeta.stage.replaceAll("_", " ")}` : ""}</span><p>{loading ? "Preparing…" : question}</p><div className="audio-wave">{Array.from({ length: 18 }, (_, i) => <i key={i} style={{ height: `${12 + ((i * 17) % 28)}px` }} />)}</div></div>
      </section>
      <footer className="room-controls">
        <button type="button" className={muted ? "control active" : "control"} onClick={() => setMuted(!muted)}>◉ {muted ? "Unmute" : "Mute"}</button>
        <button type="button" className={voiceEnabled ? "control active" : "control"} onClick={() => { setVoiceEnabled((value) => !value); if (voiceEnabled && "speechSynthesis" in window) window.speechSynthesis.cancel(); }}>🔊 {voiceEnabled ? "Voice on" : "Voice off"}</button>
        {speechSupported && <button type="button" className={listening ? "control active" : "control"} onClick={() => listening ? stopListening() : startListening()}>🎙️ {listening ? "Listening…" : "Speak answer"}</button>}
        <span className="room-remaining">{remainingLabel} remaining</span>
        <textarea value={answer} onChange={(e) => setAnswer(e.target.value)} placeholder={listening ? "Listening — speak naturally…" : "Type your answer or use Speak answer…"} disabled={ending} />
