...
export default function InterviewRoomPage() {
  ...
  useEffect(() => {
    ...
    async function startFallback() {
      try {
        ...
        setQuestion(ai.question); setQuestionMeta(ai); lastTurnSequenceRef.current = 0; speakQuestion(ai.question);
      ...
      } catch (error) { console.error(error); const fallback = "Tell me about a project you are most proud of and the impact you had on it."; setQuestion(fallback); setQuestionMeta({ question: fallback, stage: "intro", question_index: 0, role: "technical_interviewer", source: "fallback" }); speakQuestion(fallback); }
    }
    void startFallback();
  ...
  }, [interviewId, markInterviewerSpeaking]);
  ...
  async function submitAnswer() {
    ...
    setQuestion(ai.question); setQuestionMeta(ai); setAnswer(""); speakQuestion(ai.question);
  ...
  const onActiveSpeaker = useCallback((identity: string | null) => { setActiveSpeaker(identity); if (identity) markInterviewerSpeaking(true); }, [markInterviewerSpeaking]);
      ...
      <div className="room-v2-body">
        ...
        <section className="room-v2-main">
          <div className={`panel-grid panel-grid-${panelSize}`}>
            {panel.map((member, index) => <PanelVideo key={member.name} track={remoteVideos[Object.keys(remoteVideos)[index]] ?? null} name={member.name} role={member.role} accent={member.accent} active={activeSpeaker === Object.keys(remoteVideos)[index] || (!Object.keys(remoteVideos).length && interviewerSpeaking && index === 0)} />)}
          </div>
          ...
          <div className="room-v2-controls">
            ...
            <button type="button" onClick={() => void navigator.mediaDevices?.getDisplayMedia?.({ video: true }).then((screen) => screen.getTracks().forEach((track) => track.stop())).catch(() => undefined)}><span>▣</span>Share Screen</button>
            <button type="button" onClick={() => setShowTranscript((value) => !value)}><span>▢</span>Chat</button>
            <div className="room-v2-more-wrap"><button type="button" onClick={() => setShowMore((value) => !value)}><span>•••</span>More</button>{showMore && <div className="room-v2-more-menu"><span>Live camera analysis</span><span>AI interviewer video</span><span>Adaptive follow-ups</span></div>}</div>
