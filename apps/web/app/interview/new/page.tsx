...
export default function NewInterviewPage() {
  ...
  return <main className="product-shell"><nav className="product-nav container"><Link href="/dashboard" className="brand"><span className="brand-mark">IP</span><span>Interview Platform</span></Link><Link href="/dashboard" className="text-link">← Dashboard</Link></nav><section className="setup container"><div className="setup-intro"><div className="eyebrow"><span /> Interview setup</div><h1>Configure your interview.</h1><p>The engine will use these choices to build the session. You can change them before every practice interview.</p></div><div className="setup-layout"><div className="setup-main"><div className="setup-section"><label>Interview type</label><div className="choice-grid">{types.map(([id, title, description]) => <button type="button" key={id} className={`choice-card ${type === id ? "selected" : ""}`} onClick={() => setType(id)}><span>{id === type ? "✓" : "○"}</span><strong>{title}</strong><small>{description}</small></button>)}</div></div><div className="setup-section"><label>Difficulty</label><div className="segmented">{["Easy", "Adaptive", "Hard"].map((item) => <button type="button" key={item} className={difficulty === item ? "active" : ""} onClick={() => setDifficulty(item)}>{item}</button>)}</div></div><div className="setup-section"><label>Interview duration</label><div className="segmented">{["15 min", "30 min", "45 min"].map((item) => <button type="button" key={item} className={duration === item ? "active" : ""} onClick={() => setDuration(item)}>{item}</button>)}</div></div><div className="setup-section"><label>Interview language</label><div className="segmented">{["English", "Hindi", "Hinglish"].map((item) => <button type="button" key={item} className={language === item ? "active" : ""} onClick={() => setLanguage(item)}>{item}</button>)}</div></div></div><aside className="setup-summary"><span className="eyebrow">Session preview</span><h2>{types.find(([id]) => id === type)?.[1]}</h2><p>{types.find(([id]) => id === type)?.[2]}</p><div className="summary-list"><div><span>Difficulty</span><b>{difficulty}</b></div><div><span>Duration</span><b>{duration}</b></div><div><span>Language</span><b>{language}</b></div><div><span>Panel</span><b>{panel}</b></div></div><div className="summary-note"><strong>AI context</strong><span>Your profile and resume context will be available to the interviewer.</span></div><Link href={`/interview/check?type=${type}&difficulty=${difficulty.toLowerCase()}&duration=${duration.replace(" ", "-")}`} className="button button-primary button-large full-width">Continue to device check <span>→</span></Link></aside></div></section></main>;
}
