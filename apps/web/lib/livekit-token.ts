...
export function createLiveKitToken(input: {
  ...
  const agents = Array.from({ length: panelSize }, (_, index) => ({
    agentName: `interview-agent-${index + 1}`,
    metadata: JSON.stringify({ interviewId: input.interviewId ?? "", panelIndex: index, panelRole: PANEL_ROLES[index] ?? PANEL_ROLES[0] }),
