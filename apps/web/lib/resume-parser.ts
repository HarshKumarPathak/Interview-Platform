...
function lines(text: string) {
  return text.slice(0, MAX_TEXT).split(/\r?\n/).map(clean).filter(Boolean);
}
...
function listItems(values: string[]) {
  return values.flatMap((line) => line.split(/\s*[•▪●]\s*|\s*;\s*/).map(clean)).filter(Boolean);
}
...
export function parseResumeText(input: string): ResumeContext {
  ...
  const education = listItems(sections.education ?? []).map((value) => ({
    institution: value,
    year: value.match(/\b(?:19|20)\d{2}\b/)?.[0],
  }));

  const experience = listItems([...(sections.experience ?? []), ...(sections["work experience"] ?? [])]).map((value) => ({
    role: value.split(/\s+at\s+|\s+@\s+/i)[0]?.trim(),
    company: value.match(/\s+(?:at|@)\s+(.+)$/i)?.[1]?.trim(),
    dates: value.match(/\b(?:19|20)\d{2}\s*[-–]\s*(?:19|20)\d{2}|\b(?:19|20)\d{2}\s*[-–]\s*(?:Present|Current)\b/i)?.[0],
    bullets: [value],
  }));
  ...
  return {
    contact: { name, email, phone, links: urls.slice(0, 10) },
    summary: clean([...(sections.summary ?? []), ...(sections.profile ?? [])].join(" ")) || undefined,
    education,
    experience,
    projects,
    skills: [...new Set(skills)].slice(0, 100),
    certifications: listItems(sections.certifications ?? []).slice(0, 50),
    achievements: listItems(sections.achievements ?? []).slice(0, 50),
  };
}
